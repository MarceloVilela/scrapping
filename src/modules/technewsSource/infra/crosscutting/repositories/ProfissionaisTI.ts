...
class ProfissionaisTI implements IArticlesRepository {
  ...
  async getHome(): Promise<IResponseHomeDTO> {
    ...
    console.log(`@ProfissionaisTI/getHome()/url:${url}`);
    ...
    const getContent = (elPost: Element) => {
      return {
        ...
        thumb: elPost.querySelector('img.post_thumbnail')?.getAttribute('data-src'),
  ...
  async getPost({ url }: IShowPostDTO): Promise<Article> {
    console.log(`@ProfissionaisTI/getPost()/url:${url}`);
    const response = await JSDOM.fromURL(url);
    const { document } = response.window;
