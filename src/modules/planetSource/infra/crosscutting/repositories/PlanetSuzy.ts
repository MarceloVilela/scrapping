...
import IPostsRepository from '@modules/planetSource/repositories/IPostRepository';
import IShowPostDTO from '@modules/planetSource/dtos/IShowPostDTO';
import AppError from '@shared/errors/AppError';
import Post from '../schemas/Post';

class PlanetSuzy implements IPostsRepository {
  getOriginUrl() {
  ...
  async getPost({ url }: IShowPostDTO): Promise<Post> {
    ...
    const getContent = elPost => {
      const sourceLink = elPost
        .querySelector('a[id^="postcount"]')
        .getAttribute('href');

      const message = elPost.querySelector('div[id^="post_message"]')
        .textContent;
      ...
      const id = elPost
        .querySelector('a[id^="postcount"]')
        .textContent.padStart(4, '0');

      const textContent = message
        .split('\n')
        .filter(val => !val.includes('http') && val !== '' && val !== ' ')
        .join('\n');

      const contents = [{ text: textContent, id }];

      const posted_at = elPost
        .querySelector('td.thead:nth-of-type(1)')
        .textContent.replace(/\n/g, '')
        .replace(/\t/g, '');
    ...
    if (document.querySelector('.pagenav .alt1:last-of-type a') !== null) {
      pageLast = dom.window.document
        .querySelector('.pagenav .alt1:last-of-type a')
        .getAttribute('href')
        .split('=')
        .pop();
    }
    ...
    if (document.querySelector('.pagenav .vbmenu_control') !== null) {
      pageLast = document
        .querySelector('.pagenav .vbmenu_control')
        .textContent.split(' ')
        .pop();
    }
